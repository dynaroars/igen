...
def str_of_mcores_d_lens(mcores_d):
    if vdebug:
        assert is_mcores_d(mcores_d),mcores_d
    ...
    for siz in sorted(set(sizs)):
        ...
        for c in siz_conds:
            for sid in mcores_d[c]:
                s.add(sid)
        ss.append("({},{},{})".
                  format(siz,len(siz_conds),len(s)))
...
def print_iter_stat(robj):
    (citer,etime,samples,covs,new_covs,new_cores,rcore,cores_d) = robj

    logger.info("iter {} ({}) ".format(citer,etime) +
    ...
    logger.debug('mcores\n{}\n{}'
                 .format(str_of_mcores_d(mcores_d),
                         str_of_mcores_d_lens(mcores_d)))
    logger.info('****************************************\n')

def str_of_summary(seed,iters,ntime,nsamples,ncovs,tmpdir):
    s = ("Summary: Seed {}, Iters {}, Time {}, Samples {}, Covs {}, Tmpdir '{}'"
         .format(seed,iters,ntime,nsamples,ncovs,tmpdir))
    return s
...
def replay(dirname):
    ...
    for robj in robjs:
        if len(robj) == 8:
            ...
            (citer,etime,samples,covs,
             new_covs,new_cores,cores_d) = robj
            rcore = (None,None)
            robj = (citer,etime,samples,covs,
    ...
    logger.info(str_of_summary(seed,niters,
                               ntime,nsamples,ncovs,dirname))

    return niters,ntime,nsamples,ncovs
...
def iterative_refine(dom,get_cov,seed=None,tmpdir=None,pure_random_n=None,config_default=None):
    if vdebug:
        assert is_dom(dom),dom
    ...
    samples,covs = eval_samples(samples,get_cov,configs_cache)
    ...
    logger.info(str_of_summary(seed,cur_iter,time()-st,
                               len(configs_cache),len(cores_d),tmpdir))
    return cores_d
...
def implies(x,y):
    ...
    if x is None:
        return True  #False -> *
    if y is None:
        return False #  ** -> False where ** is not None

    return x.hcontent.issubseteq(y.hcontent)
    ...
    for core_pn in mcores_d:
        ...
        for core_pn_ in mcores_d:
            if core_pn != core_pn_:
                ...
                if implies(core_p,core_p_) and implies(core_n_,core_n):
...
def benchmark_stats(results_dir):
    niters_total = 0
    ntime_total = 0
    nsamples_total = 0
    ncovs_total = 0
    ...
    for rdir in results_dir:
        niters,ntime,nsamples,ncovs = replay(rdir)
        niters_total += niters
        ntime_total += ntime
    ...
    logger.info("avg: "
                ...
                "samples {}".format(samples_total/nruns_total)
                "covs {}".format(covs_total/nruns_total)
                "runs {}".format(runs_total/nruns_total)                
                )
