import settings
from config import (DTrace, Dom, Config, Core, SCore, PNCore, Cores_d)
...
class Infer(object):
    ...
    def infer(self, configs, core) -> Core:
        """
    ...
    def infer_cache(self, core, configs, cache):
        ...
        key = (core, configs)
        if key not in cache:
            cache[key] = self.infer(configs, core)
        return cache[key]

    def infer_sid(self, sid, core, cconfigs_d, cache):
        assert isinstance(sid, str), sid
        assert isinstance(core, PNCore), (core, type(core))
        ...
        def _f(configs, cc, cd, _b):
            new_cc, new_cd = cc, cd
            if configs:
                new_cc = self.infer_cache(cc, configs, cache)
            ...
            if new_cc:
                ...
                if configs_:
                    new_cd = self.infer_cache(cd, configs_, cache)
                    if new_cd:
    ...
    def infer_covs(self, cconfigs_d, sids=None) -> tuple:
        ...
        for sid in sorted(mysids):
            ...
            icore = self.infer_sid(sid, core, cconfigs_d, cache)
