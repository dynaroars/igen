...
def get_run_f(args, mlog) -> tuple:
    ...
    and call appropriate mbuild function on those inputs
    """
    import config as DS
    dom, default_configs = DS.Dom.get_dom(args.dom_dir)

    import get_cov

    def get_cov_f(config):
        return get_cov.runscript_get_cov(config, dom.run_script)

    sids = get_sids(args.sids)
    econfigs = [(c, None) for c in default_configs] if default_configs else []

    import alg as ALG
    mbuild = ALG.MBuild(dom, get_cov_f, sids)

    if sids:
        def run_f(seed, tdir): return mbuild.go(
            seed=seed, econfigs=econfigs, tmpdir=tdir)

    elif args.cmp_rand:
        def run_f(seed, tdir, rand_n): return mbuild.go_rand(
            rand_n=rand_n, seed=seed, econfigs=econfigs, tmpdir=tdir)

    elif args.do_full:
        def run_f(_, tdir): return mbuild.go_full(tmpdir=tdir)
        ...
        def run_f(seed, tdir): return mbuild.go_rand(
...
if __name__ == "__main__":
    ...
    mbuild_file = CM.getpath(__file__)
    mbuild_name = os.path.basename(mbuild_file)
    mbuild_dir = os.path.dirname(mbuild_file)
    ...
    # two main modes:
    # 1. run mbuild to find interactions and
    # 2. run Analysis to analyze mbuild's generated files
    analysis_f = None
    ...
    if not analysis_f:  # run mbuild
        run_f, get_cov_f = get_run_f(args, mlog)

        prefix = "mbuild_{}_{}_".format(
        ...
        if cmp_rand:
            ...
            tdir = tempfile.mkdtemp(dir=settings.tmp_dir,
                                    prefix=cmp_rand + "mbuild_cmp_rand")
