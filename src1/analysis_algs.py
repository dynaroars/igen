...
class MinConfigs(XAnalysis):
    ...
    def get_minset_configs_d(self, remain_covs):
        ...
        # some init setup
        exprs_d = [(c, c.z3expr(self.ld.z3db))
                   for c in self.ld.configs_d]  # slow
        ...
        def _g(pack, e_configs):
            ...
            configs = self.ld.dom.gen_configs_expr(
                expr, k=1, config_cls=IA.Config)
            if not configs:
                return None
            else:
                config = configs[0]
...
class Influence(XAnalysis):
    ...
    def search(self, ncovs, do_settings=True):
        ...
        def _f(k, v): 
            return v, k
        rs.sort(key=_f, reverse=True)
        rs = [(k, v, 100. * v / ncovs) for k, v in rs]

        def _g(k, v, p):
            return "{}, {}, {}".format(_str(k), v, p)

        print("influence (opt, uniq, %) {}"
              .format(', '.join(map(g, rs))))
        return rs
            ...
            for cov in covs:
                if cov not in self.ld.ncovs_d:  # no neg configs for cov
                    ...
                    if expr is None:  # true interaction but some locs are not covered
                        ...
                        stat = check(
                            self.ld.ncovs_d[cov], nexpr, cache, solver)
        ...
        for pncore in self.ld.mcores_d:
            ...
            for cov in covs:
                if expr is None:
                    ...
                    stat = check(cd.ccovs_d[cov], expr,
                                 cache, self.ld.z3db.solver)
